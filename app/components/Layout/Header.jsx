...
class Header extends React.Component {
	...
	render() {
		...
		let dashboard = (
            ...
            >
                <img style={{margin: 0, height: 40}} src={require('./logo1.png')} />
            </a>
		);
		...
		onClick = {
			this._onNavigate.bind(this, "/market/USD_BTS")
		} > < Translate component = "span"
		content = "header.exchange" / > < /a>;

		// Account selector: Only active inside the exchange
		let accountsDropDown = null,
			account_display_name, accountsList;
		if(currentAccount) {
		...
		let settingsDropdown = <ActionSheet>
            ...
            <ActionSheet.Content>
                <ul className="no-first-element-top-border">
                    ...
                    {/*交易直接入口*/}
                    <li>
                        <a href onClick={this._onNavigate.bind(this, "market/BMAN_JRC")}>
                            <span>交易</span>
                        </a>
                    </li>
                    {/*充值页*/}
                    <li>
                        <a href onClick={this._onNavigate.bind(this, "/recharge")}>
                            <span>充值</span>
                        </a>
                    </li>
                    {/*提现页*/}
                    <li>
                        <a href onClick={this._onNavigate.bind(this, "/cash")}>
                            <span>提现</span>
                        </a>
                    </li>
                    <li>
                ...
                <div className="grid-block show-for-medium">
                    <ul className="menu-bar">
                       ...
                       {/*currentAccount || myAccounts.length ? <li><a className={cnames({active: active.indexOf("transfer") !== -1})} onClick={this._onNavigate.bind(this, "/lock")}><Translate component="span" content="header.deposit" /></a></li> : null*/}
                        {/*交易直接入口*/}
                        {!(currentAccount || myAccounts.length) ? <li><a className={cnames({active: active.indexOf("market/BMAN_JRC") !== -1})} onClick={this._onNavigate.bind(this, "/market/BMAN_JRC")}>交易</a></li> : null}
                        {/*充值入口*/}
                        {!(currentAccount || myAccounts.length) ? <li><a className={cnames({active: active.indexOf("explorer") !== -1})} onClick={this._onNavigate.bind(this, "/recharge")}>充值</a></li> : null}
                        {/*提现入口*/}
                        {!(currentAccount || myAccounts.length) ? <li><a className={cnames({active: active.indexOf("Cash") !== -1})} onClick={this._onNavigate.bind(this, "/cash")}>提现</a></li> : null}
                    ...
                    <div className="my-login">
                    	{!(currentAccount || myAccounts.length) ? <p><a className={cnames({active: active.indexOf("create-account/wallet") !== -1})} onClick={this._onNavigate.bind(this, "/create-account/wallet")}>账号登录</a></p> : null}
                    </div>
                </div>
